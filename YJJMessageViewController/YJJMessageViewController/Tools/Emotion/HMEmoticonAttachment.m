...
- (instancetype)initWithEmoticon:(HMEmoticon *)emoticon font:(UIFont *)font {
    ...
    if (self) {
        ...
        if (!self.image) {
            self.image=[UIImage qm_imageNamed:emoticon.imagePath];
        }
...
+(NSAttributedString*)emoticonStringWithEmoticon:(HMEmoticon *)emoticon WithDic:(NSDictionary *)dic
{
    HMEmoticonAttachment *attachment = [[HMEmoticonAttachment alloc] initWithEmoticon:emoticon font:dic[NSFontAttributeName]];
    
    NSMutableAttributedString *emoticonStr = [[NSMutableAttributedString alloc] initWithAttributedString:
                                              [NSAttributedString attributedStringWithAttachment:attachment]];
    [emoticonStr addAttributes: @{NSFontAttributeName:dic[NSFontAttributeName],
                                  NSForegroundColorAttributeName: dic[NSForegroundColorAttributeName],NSParagraphStyleAttributeName:dic[NSParagraphStyleAttributeName]}
                         range:NSMakeRange(0, 1)];
