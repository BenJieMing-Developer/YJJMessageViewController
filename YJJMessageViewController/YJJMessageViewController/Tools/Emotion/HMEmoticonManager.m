...
- (NSAttributedString *)emoticonStringWithString:(NSString *)string font:(UIFont *)font textColor:(UIColor *)textColor {
    ...
    for (NSTextCheckingResult *result in matches.reverseObjectEnumerator) {
        ...
        if (emoticon != nil) {
            NSAttributedString *emoticonString = [HMEmoticonAttachment emoticonStringWithEmoticon:emoticon font:[UIFont systemFontOfSize:18] textColor:textColor];
...
#pragma mark - 表情和字符串分别自定义设置
- (NSAttributedString *)emoticonStringWithString:(NSString *)Str WithEmojiDic:(NSDictionary *)dic StrDic:(NSDictionary *)dic1
{
    ...
    NSString *pattern = @"\\[.*?\\]";
    NSRegularExpression *regx = [NSRegularExpression regularExpressionWithPattern:pattern options:0 error:NULL];
    NSArray *matches = [regx matchesInString:Str options:0 range:NSMakeRange(0, Str.length)];
    for (NSTextCheckingResult *result in matches.reverseObjectEnumerator) {
        ...
        HMEmoticon *emoticon = [self emoticonWithString:str];
        if (emoticon != nil) {
            NSAttributedString *emoticonString =[HMEmoticonAttachment emoticonStringWithEmoticon:emoticon WithDic:dic];
            [attributedString replaceCharactersInRange:range withAttributedString:emoticonString];
           
        }
        else
        {
            [attributedString addAttributes:dic1 range:range];
        }
    }
